...
- **Auto-split**: Meta-package with `Depends:` on all parts (version-pinned), each part as separate DEB. Uses even-parts sizing (divides total size equally across parts) with 80% safety factor on format limits
...
- **spm-deb**: 58 tests (ar: 14, control: 16, builder: 20, reader: 8)
